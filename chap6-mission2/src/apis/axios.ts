...
axiosInstance.interceptors.request.use(
  (config) => {
    ...
    // accessToken이 존재하면 Authorization 헤더에 Bearer 토큰을 추가한다.
    if (accessToken) {
    ...
    if (
      ...
      if (!refreshPromise) {
        ...
        refreshPromise = (async () => {
          ...
          const refreshToken = getRefreshToken(); // localStorage에서 refreshToken을 가져옴옴

          const { data } = await axiosInstance.post("/v1/auth/refresh", {
            refresh: refreshToken,
          });

          // data에서 새 토큰을 가져와서 반환
          ...
          setAccessToken(data.data.accessToken); // 새 accessToken 저장
          setRefreshToken(data.data.refreshToken); // 새 refreshToken 저장
        ...
        .finally(() => {
          refreshPromise = null; // 요청이 끝나면 프라미스 초기화
        });
      }
